	...
	static {
		...
		try {
			...
			Document document = reader.read(new FileInputStream(new File(
					ExcelToObj.class.getClassLoader()
			...
			dataBegin.load(new FileInputStream(new File(ExcelToObj.class
					.getClassLoader().getResource("excel_dataBegin.properties")
					.getPath())));
			System.out.println();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void test() {
		...
		try {
			...
			for (Element element : element_excel) {
				...
				String excelFileName = element.attribute("name").getName();
				List<Element> elements_excel = element.elements("object");

				for (Element element2 : elements_excel) {
					List<Element> elements = element2.elements();
					for (Element element3 : elements) {
						String key = element3.getName();
